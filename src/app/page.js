...
export default function Home() {
  ...
  const fetchProduct = async () => {
    try {
      const res = await axios.get(`tech0-gen8-step4-pos-app-36.azurewebsites.net/product/${barcode}`);
      if (res.data.product) {
  ...
  const addToCart = async () => {
    if (!product) return;
    try {
      const res = await axios.post("tech0-gen8-step4-pos-app-36.azurewebsites.net/cart/add", {
  ...
  const purchase = async () => {
    ...
    try {
      const res = await axios.post("tech0-gen8-step4-pos-app-36.azurewebsites.net/purchase", requestData);
